    ...
    public void run() {
        ...
        for (World w : updates.keySet()) {
            if (w != null) {
                ...
                ArrayList<net.minecraft.server.v1_12_R1.Chunk> chunksToRelight = new ArrayList<net.minecraft.server.v1_12_R1.Chunk>();
                net.minecraft.server.v1_12_R1.World nativeWorld = ((CraftWorld) w).getHandle();
                ...
                for (int mapUpdateIndex = 0; mapUpdateIndex < updatesInWorld.size(); mapUpdateIndex++) { // TODO: make this go in chunks instead of block by block, same with the block placement system
                    ...
                    if (i != null) {
                        if (i.getTypeID() >= 0 && i.getWorldEditBaseBlock() == null && i.getOldBlockLocation() != null) {
                            ...
                            net.minecraft.server.v1_12_R1.Chunk nativeSrcChunk = ((CraftChunk) srcBlock.getChunk()).getHandle();
...
//								}
                    ...
                    if (i != null) {
                        if (i.getTypeID() >= 0) {
                            if (i.getWorldEditBaseBlock() == null) {
                                ...
                                if (!delayed) {
                                    net.minecraft.server.v1_12_R1.Chunk nativeDstChunk = ((CraftChunk) dstBlock.getChunk()).getHandle();
                                    BlockPosition dstBlockPos = new BlockPosition(dstBlock.getX(), dstBlock.getY(), dstBlock.getZ());
                                    net.minecraft.server.v1_12_R1.Chunk nativeSrcChunk = null;
                                    ...
                                    if (existingType != newType || existingData != newData) { // only place the actual block if it has changed
                                        ...
                                        net.minecraft.server.v1_12_R1.ChunkSection dstSection = nativeDstChunk.getSections()[dstBlock.getY() >> 4];
                                        if (dstSection == null) {
                                            // Put a GLASS block to initialize the section. It will be replaced next with the real block.
                                            nativeDstChunk.a(dstBlockPos, net.minecraft.server.v1_12_R1.Blocks.GLASS.getBlockData());
                                ...
                                if (entityMap.containsKey(i.getNewBlockLocation())) {
                                    ...
                                    for (EntityUpdateCommand entityUpdate : mapUpdateList) {
                                        ...
                                        if (entity instanceof Player) {
                                            net.minecraft.server.v1_12_R1.EntityPlayer craftPlayer = ((CraftPlayer) entity).getHandle();
                ...
                for (MapUpdateCommand i : updatesInWorld) {
                    if (i != null) {
                        ...
                        net.minecraft.server.v1_12_R1.Chunk nativeDstChunk;
                        if (i.getOldBlockLocation() != null) {
...
/*				// send updates to clients
                ...
                if (!Settings.CompatibilityMode) {
                    for (net.minecraft.server.v1_12_R1.Chunk c : chunksToRelight) {
        ...
        if (sendSil) {
            for (Player p : w.getPlayers()) { // this is necessary because signs do not get updated client side correctly without refreshing the chunks, which causes a memory leak in the clients
                ...
                if ((dist < Settings.SilhouetteViewDistance * Settings.SilhouetteViewDistance) && (dist > 32 * 32)) {
                    net.minecraft.server.v1_12_R1.EntityPlayer craftPlayer = ((CraftPlayer) p).getHandle();
